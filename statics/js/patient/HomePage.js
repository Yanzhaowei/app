     ...
     this.setState({ 
      Lvisible:true,
       modalStyle:{
         ...
         width:100,
         backgroundColor: 'rgba(255, 255, 255,0.7)',},
       modalContent:<FLModal  classify={(name)=>this.classify(name)} navigator={this.props.navigator} name='FLModal' close={()=>this.closeModal()} openCenter={()=>this.openCenterModal()}/> ,
    });
  }
  else{
       this.setState({Lvisible:false});
